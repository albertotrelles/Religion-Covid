...
foreach word in God Prayer Meditation {
	...
	forval i = 1/`N' {
		...
		*--- Low religion ---*
		...
		cap mean att_`var'_low [pw=pop] if ypost0_t1 & high_religion==0
		if !_rc mat `word'_low[`i', 1] = r(table)[1,1]

		cap mean att_`var'_low [pw=pop] if ypost1_t1 & high_religion==0
		if !_rc mat `word'_low[`i', 2] = r(table)[1,1]

		cap mean att_`var'_low [pw=pop] if ypost2_t1 & high_religion==0
		if !_rc mat `word'_low[`i', 3] = r(table)[1,1]
		...
		*--- High religion ---*
		...
		cap mean att_`var'_high [pw=pop] if ypost0_t1 & high_religion==1
		if !_rc mat `word'_high[`i', 1] = r(table)[1,1]

		cap mean att_`var'_high [pw=pop] if ypost1_t1 & high_religion==1
		if !_rc mat `word'_high[`i', 2] = r(table)[1,1]

		cap mean att_`var'_high [pw=pop] if ypost2_t1 & high_religion==1
		if !_rc mat `word'_high[`i', 3] = r(table)[1,1]
...
*cap + if !_rc: keeps matrix entry missing when a draw has no observations for that window × religion group combination
...
foreach word in God Prayer Meditation {
	...
	*--- Low: point estimate and 95% CI (sum ignores missing draws) ---*
	...
	*--- High: point estimate and 95% CI (sum ignores missing draws) ---*
...
foreach word in God Prayer Meditation {
	...
	*Post-periods: num 13-15 → event_time 0, 1, 2
	...
	graph export "$figures/eventstudy_het_`word'.png", replace
